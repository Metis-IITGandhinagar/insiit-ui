                        ...
                        onPressed: () async {
                          try {
                            http.Response response = await http.get(
                                Uri.parse("${API_BASE_URL}/haircut-dates"));
                            if (response.body != null) {
                              final haircutDates =
                                  jsonDecode(response.body)["data"]["dates"]
                                      .map((date) {
                              ...
                              final DateTime? pickedDate = await showDatePicker(
                                  ...
                                  selectableDayPredicate: (date) {
                                    return haircutDates.contains(date)
                                        ? true
                                        : false;
                                  });
                              if (pickedDate == null) {
                                ...
                                final pickedDateIndex =
                                    haircutDates.indexOf(pickedDate);
                                Navigator.push(
                                  context,
                                  MaterialPageRoute(
                                      builder: (context) => TimeSlotPage(
                                          initialDateIndex: pickedDateIndex)),
                                );
                              }
                            }
                          } catch (e) {
